...
def get_latest_hour():
    ...
    return str(latest_hour['traffic_duration'])[:-6]
...
def get_latest_date():
    ...
    return str(latest_date['traffic_duration']).split(" ")[0]
...
def top_traffic_domain_hour(query_date):
    ...
    sql =  "SELECT query_domain,COUNT(query_id),COUNT(DISTINCT(query_client_ip)) FROM queries "\
    ...
    sql =  "SELECT query_client_ip,COUNT(query_id),COUNT(DISTINCT(query_domain)) FROM queries "\
        "WHERE query_time BETWEEN '%s' AND '%s'"\
    ...
    sql = "SELECT COUNT(query_id) FROM queries "\
...
def top_traffic_domain_day(query_date):
    ...
    sql =  "SELECT query_client_ip,COUNT(query_id),COUNT(DISTINCT(query_domain)) FROM queries "\
        " WHERE query_time BETWEEN '%s' AND '%s' GROUP BY query_client_ip ORDER BY count(query_id) DESC LIMIT 500;"%(start, end)
    cursor.execute(sql)
...
def record_domain_control(query_date, infos, duration_flag):
    for info in infos:
        ...
        sql = "SELECT DISTINCT query_domain FROM queries WHERE query_fld=%s"
        ...
        if not has_previous_record:
            ...
            res = {
                ...
                "subdomain_limit" : active_sub_domains,
                "visitor_limit" : visitors if duration_flag == "hourly" else math.floor(visitors/6),
            ...
            if duration_flag == "daily":
                cur = pre["daily_history"] + [{
                    ...
                    "visitors": visitors,
                    "active_sub_domains": active_sub_domains}]
                traffic_limit = domain_limit.calculate_traffic_limit(pre["traffic_limit"], cur, [])
